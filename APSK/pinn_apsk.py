# Script for training 16-APSK PINN for multi-signal-type prediction
...
check_path = os.path.join(os.getcwd(), "APSK", "pinn_apsk.py")
if not os.path.isfile(check_path):
    logger.log(logging.ERROR, f"Current working directory is not project root. Expected to find 'APSK/pinn_apsk.py' at {check_path}. Please change to project root and rerun.")
    sys.exit()
else:
    logger.log(logging.INFO, f"Current working directory verified as project root: {os.getcwd()}.")

# check results directory exists
results_dir = os.path.join(os.getcwd(), "APSK", "results")
...
logger.log(logging.INFO, "Script started for training 16-APSK PINN.")
...
def main():
    parser = argparse.ArgumentParser(description="16-APSK PINN for Optical Fibre Communication.")
    ...
    parser.add_argument("--load_path", type=str, default="apsk_pinn_checkpoint.pth", help="String. Path to model checkpoint for loading. Must be within the results directory. Default 'complex_pinn_checkpoint.pth'.")
    ...
    parser.add_argument("--checkpoint_path", type=str, default="apsk_pinn_checkpoint.pth", help="String. Path to save model checkpoint. Must be within the results directory. Default 'complex_pinn_checkpoint.pth'.")
    ...
    parser.add_argument("--onnx_path", type=str, default="apsk_raw.onnx", help="String. Path to save ONNX model. Must be within the results directory. Default 'apsk_raw.onnx'.")
    ...
    assertlog(args.onnx_path != "apsk_FINN_ready.onnx", "ONNX export path cannot be 'apsk_FINN_ready.onnx' to avoid overwriting FINN conversion output. Please specify a different name.")
    ...
    if args.load_inputs:
        ...
        # Generate 16-APSK (DVB-S2: 4 inner + 12 outer)
        r1, r2 = 1.0, 2.53
        inner = r1 * np.exp(1j * np.linspace(0, 2*np.pi, 4, endpoint=False))
        outer = r2 * np.exp(1j * (np.linspace(0, 2*np.pi, 12, endpoint=False) + np.pi/12))
    ...
    if args.metrics:
        ...
        # Calculate SER
        _, clean_idx_dist = classify_16apsk(clean_dist_n)
        _, clean_idx_dbp  = classify_16apsk(clean_dbp_n)
        _, clean_idx_pinn = classify_16apsk(clean_pinn_n)

        _, dist_idx  = classify_16apsk(dist_f)
        _, ssfm_idx  = classify_16apsk(dbp_f)
        _, pinn_idx  = classify_16apsk(pinn_f)
        ...
        if args.visual:
            ...
            titles = ["Distorted (16-APSK)", "SSFM Baseline", "8-bit PINN"]
            for i in range(3):
    ...
    if args.finn_convert and args.onnx_export:
        ...
        try:
            ...
            conv2finn(dir_path=results_dir, qonnx_name=args.onnx_path, output_name="apsk_FINN_ready.onnx")
            logger.log(logging.INFO, "Successfully converted QONNX model to FINN format as apsk_FINN_ready.onnx.")
        except Exception as e:
