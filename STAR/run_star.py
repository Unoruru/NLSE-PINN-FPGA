# Script to run the Star-QAM PINN training and evaluation process for reinforcement learning.
...
with open("pinn_star.log", "w") as log_file: # Clear log file at the start of each run
    log_file.write("")

logger.log(logging.INFO, "Log saved as pinn_star.log in current working directory.")

from pinn_star import assertlog, str2bool

def main():
    parser = argparse.ArgumentParser(description="Run Star-QAM PINN Training and Evaluation with Reinforcement Learning.")
    ...
    parser.add_argument("--script_path", type=str, default="STAR/pinn_star.py", help="String. Path to the Star-QAM PINN training script. Default 'STAR/pinn_star.py'.")
