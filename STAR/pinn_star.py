# Script for training Star-QAM PINN for multi-signal-type prediction
...
file_handler = logging.FileHandler('pinn_star.log', mode='a') # appends to log file each run (a append w overwrite)
...
logger.log(logging.INFO, "Log saved as pinn_star.log in current working directory.")

# Check if current working directory is correct (should be project root)
check_path = os.path.join(os.getcwd(), "STAR", "pinn_star.py")
if not os.path.isfile(check_path):
    logger.log(logging.ERROR, f"Current working directory is not project root. Expected to find 'STAR/pinn_star.py' at {check_path}. Please change to project root and rerun.")
    sys.exit()
else:
    logger.log(logging.INFO, f"Current working directory verified as project root: {os.getcwd()}.")

# check results directory exists
results_dir = os.path.join(os.getcwd(), "STAR", "results")
...
logger.log(logging.INFO, "Script started for training Star-QAM PINN.")
...
# Training loop for the Star-QAM PINN
def train(model, device, X_train, Y_train, epochs, lr, beta2, gamma, scale_factor):
    optimizer = optim.Adam(model.parameters(), lr=lr)

    train_start = time.time()

    with trange(epochs, desc="Training Star-QAM PINN") as t:
        for e in t:
            model.train()
            optimizer.zero_grad()
...
def main():
    parser = argparse.ArgumentParser(description="Star-QAM PINN for Optical Fibre Communication.")
    ...
    parser.add_argument("--load_path", type=str, default="star_pinn_checkpoint.pth", help="String. Path to model checkpoint for loading. Must be within the results directory. Default 'complex_pinn_checkpoint.pth'.")
    ...
    parser.add_argument("--checkpoint_path", type=str, default="star_pinn_checkpoint.pth", help="String. Path to save model checkpoint. Must be within the results directory. Default 'complex_pinn_checkpoint.pth'.")
    ...
    parser.add_argument("--onnx_path", type=str, default="star_raw.onnx", help="String. Path to save ONNX model. Must be within the results directory. Default 'star_raw.onnx'.")
    ...
    assertlog(args.onnx_path != "star_FINN_ready.onnx", "ONNX export path cannot be 'star_FINN_ready.onnx' to avoid overwriting FINN conversion output. Please specify a different name.")
    ...
    if args.load_inputs:
        ...
        # Generate Star-QAM (two rings of 8, r2/r1 = 2.0)
        r1, r2 = 1.0, 2.0
        inner = r1 * np.exp(1j * np.linspace(0, 2*np.pi, 8, endpoint=False))
        outer = r2 * np.exp(1j * (np.linspace(0, 2*np.pi, 8, endpoint=False) + np.pi/8))
    ...
    if args.metrics:
        ...
        # Calculate SER
        _, clean_idx_dist = classify_star_qam(clean_dist_n)
        _, clean_idx_dbp  = classify_star_qam(clean_dbp_n)
        _, clean_idx_pinn = classify_star_qam(clean_pinn_n)

        _, dist_idx  = classify_star_qam(dist_f)
        _, ssfm_idx  = classify_star_qam(dbp_f)
        _, pinn_idx  = classify_star_qam(pinn_f)
        ...
        if args.visual:
            ...
            titles = ["Distorted (Star-QAM)", "SSFM Baseline", "8-bit PINN"]
            for i in range(3):
    ...
    if args.finn_convert and args.onnx_export:
        ...
        try:
            ...
            conv2finn(dir_path=results_dir, qonnx_name=args.onnx_path, output_name="star_FINN_ready.onnx")
            logger.log(logging.INFO, "Successfully converted QONNX model to FINN format as star_FINN_ready.onnx.")
        except Exception as e:
